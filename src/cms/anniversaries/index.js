...
const AnniversaryTable = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                const response = await axios.get('http://localhost:3000/anniversaries', { params: query });
    ...
    const handleDelete = async (id) => {
        try {
            setLoading(true);
            await axios.delete(`http://localhost:3000/anniversaries/${id}`, {
                headers: {
            ...
            const response = await axios.get('http://localhost:3000/anniversaries', { params: query });
            setData(response.data.data);
            setToast({ show: true, type: 'success', message: 'Anniversary berhasil dihapus!' });
            setModalShow(false);
        } catch (error) {
            console.error('Error deleting item:', error);
            setToast({ show: true, type: 'error', message: 'Gagal menghapus Anniversary.' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleCreateNew = () => {
        navigate('/admin/anniversary/create');
    };

    const handleEdit = (id) => {
        navigate(`/admin/anniversary/edit/${id}`);
    };
        ...
        <>
            ...
            <div className="overflow-x-auto my-32 px-6">
                <div className="flex justify-between mb-6">
                    <h2 className="text-2xl font-semibold text-gray-800">Anniversaries</h2>
                    <button
                ...
                <table className="min-w-full bg-white shadow-md rounded-lg overflow-hidden">
                    <thead className="bg-gray-800 text-white">
                        <tr>
                            ...
                            <th className="text-left py-3 px-4 uppercase font-semibold text-sm">Link Video</th>
                        ...
                        {data.map((item) => (
                            <React.Fragment key={item.id}>
                                ...
                                >
                                    ...
                                    <td className="py-3 px-4">{item.videoLink}</td>
                                        ...
                                        <td colSpan="4" className="py-4 px-6">
                                            <div className="flex justify-between">
                                                <div>
                                                    ...
                                                    <p><span className="font-semibold">Link Video:</span> {item.videoLink}</p>
                                                </div>
                                                <button
                ...
                <ConfirmationModal
                    show={modalShow}
                    title="Hapus Anniversary"
                    message="Apakah Anda yakin ingin menghapus Anniversary ini?"
