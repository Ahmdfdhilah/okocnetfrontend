...
const UpdateAnniversary = () => {
    ...
    useEffect(() => {
        const fetchAnniversary = async () => {
            try {
                setLoading(true);
                const response = await axios.get(`http://localhost:3000/anniversaries/${id}`);
                const { judul, deskripsi, videoLink, year } = response.data;
                setFormData({
                    judul,
                    year,
                    deskripsi,
    ...
    const handleSubmit = async (e) => {
        ...
        setModalAction(() => async () => {
            try {
                setLoading(true);
                await axios.put(`http://localhost:3000/anniversaries/${id}`, formData, {
                    headers: {
        ...
        setModalMessage('Apakah Anda yakin ingin mengupdate anniversary ini?');
        setModalShow(true);
    };
    ...
    return (
        <>
            ...
            <div className="container mx-auto py-10 mt-32">
                <h1 className="text-4xl font-bold mb-8 text-center">Update Anniversary</h1>
...
export default UpdateAnniversary;