...
const TujuanMasterMentorTable = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                const response = await axios.get('http://localhost:3000/tujuan-master-mentors', { params: query });
    ...
    const handleDelete = async (id) => {
        try {
            setLoading(true);
            await axios.delete(`http://localhost:3000/tujuan-master-mentors/${id}`, {
                headers: {
            ...
            const response = await axios.get('http://localhost:3000/tujuan-master-mentors', { params: query });
            setData(response.data.data);
            setToast({ show: true, type: 'success', message: 'Tujuan Master Mentor berhasil dihapus!' });
            setModalShow(false);
        } catch (error) {
            console.error('Error deleting item:', error);
            setToast({ show: true, type: 'error', message: 'Gagal menghapus Tujuan Master Mentor.' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleCreateNew = () => {
        navigate('/admin/tujuan-master-mentor/create');
    };

    const handleEdit = (id) => {
        navigate(`/admin/tujuan-master-mentor/edit/${id}`);
    };
        ...
        <>
            ...
            <div className="overflow-x-auto my-32 px-6">
                <div className="flex justify-between mb-6">
                    <h2 className="text-2xl font-semibold text-gray-800">Tujuan Master Mentor</h2>
                    <button
...
export default TujuanMasterMentorTable;