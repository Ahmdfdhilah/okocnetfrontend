...
const SyaratMasterMentorTable = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                const response = await axios.get('http://localhost:3000/syarat-master-mentors', { params: query });
    ...
    const handleDelete = async (id) => {
        try {
            setLoading(true);
            await axios.delete(`http://localhost:3000/syarat-master-mentors/${id}`, {
                headers: {
            ...
            const response = await axios.get('http://localhost:3000/syarat-master-mentors', { params: query });
            setData(response.data.data);
            setToast({ show: true, type: 'success', message: 'Syarat Master Mentor berhasil dihapus!' });
            setModalShow(false);
        } catch (error) {
            console.error('Error deleting item:', error);
            setToast({ show: true, type: 'error', message: 'Gagal menghapus Syarat Master Mentor.' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleCreateNew = () => {
        navigate('/admin/syarat-master-mentor/create');
    };

    const handleEdit = (id) => {
        navigate(`/admin/syarat-master-mentor/edit/${id}`);
    };
        ...
        <>
            ...
            <div className="overflow-x-auto my-32 px-6">
                <div className="flex justify-between mb-6">
                    <h2 className="text-2xl font-semibold text-gray-800">Syarat Master Mentor</h2>
                    <button
                ...
                <ConfirmationModal
                    show={modalShow}
                    title="Hapus Syarat Master Mentor"
                    message="Apakah Anda yakin ingin menghapus Syarat Master Mentor ini?"
...
export default SyaratMasterMentorTable;