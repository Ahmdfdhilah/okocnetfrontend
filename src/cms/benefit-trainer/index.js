...
const BenefitTrainerTable = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                const response = await axios.get('http://localhost:3000/benefit-trainers', { params: query });
    ...
    const handleDelete = async (id) => {
        try {
            setLoading(true);
            await axios.delete(`http://localhost:3000/benefit-trainers/${id}`, {
                headers: {
            ...
            const response = await axios.get('http://localhost:3000/benefit-trainers', { params: query });
            setData(response.data.data);
            setToast({ show: true, type: 'success', message: 'Benefit Trainer berhasil dihapus!' });
            setModalShow(false);
        } catch (error) {
            console.error('Error deleting item:', error);
            setToast({ show: true, type: 'error', message: 'Gagal menghapus Benefit Trainer.' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleCreateNew = () => {
        navigate('/admin/benefit-trainer/create');
    };

    const handleEdit = (id) => {
        navigate(`/admin/benefit-trainer/edit/${id}`);
    };
        ...
        <>
            ...
            <div className="overflow-x-auto my-32 px-6">
                <div className="flex justify-between mb-6">
                    <h2 className="text-2xl font-semibold text-gray-800">Benefit Trainer</h2>
                    <button
                            ...
                            <React.Fragment key={item.id}>
                                ...
                                >
                                    ...
                                    <td className="py-3 px-4">
                                        <img className="h-10 w-10 rounded-full object-cover" src={`http://localhost:3000${item.img}`} alt={item.judul} />
                                    </td>
                                    <td className="py-3 px-4">
                                        <button
                                            onClick={(e) => {
                ...
                <ConfirmationModal
                    show={modalShow}
                    title="Hapus Benefit Trainer"
                    message="Apakah Anda yakin ingin menghapus Benefit Trainer ini?"
...
export default BenefitTrainerTable;