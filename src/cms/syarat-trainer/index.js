...
const SyaratTrainerTable = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true);
                const response = await axios.get('http://localhost:3000/syarat-trainers', { params: query });
    ...
    const handleDelete = async (id) => {
        try {
            setLoading(true);
            await axios.delete(`http://localhost:3000/syarat-trainers/${id}`, {
                headers: {
            ...
            const response = await axios.get('http://localhost:3000/syarat-trainers', { params: query });
            setData(response.data.data);
            setToast({ show: true, type: 'success', message: 'Syarat Trainer berhasil dihapus!' });
            setModalShow(false);
        } catch (error) {
            console.error('Error deleting item:', error);
            setToast({ show: true, type: 'error', message: 'Gagal menghapus Syarat Trainer.' });
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleCreateNew = () => {
        navigate('/admin/syarat-trainer/create');
    };

    const handleEdit = (id) => {
        navigate(`/admin/syarat-trainer/edit/${id}`);
    };
        ...
        <>
            ...
            <div className="overflow-x-auto my-32 px-6">
                <div className="flex justify-between mb-6">
                    <h2 className="text-2xl font-semibold text-gray-800">Syarat Trainer</h2>
                    <button
                ...
                <ConfirmationModal
                    show={modalShow}
                    title="Hapus Syarat Trainer"
                    message="Apakah Anda yakin ingin menghapus Syarat Trainer ini?"
...
export default SyaratTrainerTable;