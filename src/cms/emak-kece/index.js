...
const EmakKeceTable = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await axios.get('http://localhost:3000/emak-keces', { params: query });
    ...
    const handleDelete = async () => {
        setLoading(true);
        try {
            await axios.delete(`http://localhost:3000/emak-keces/${toBeDeletedId}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
            setToast({ show: true, type: 'success', message: 'Emak Kece berhasil dihapus!' });
            fetchData();
        } catch (error) {
            console.error('Error deleting Emak Kece:', error);
            setToast({ show: true, type: 'error', message: 'Gagal menghapus Emak Kece.' });
        } finally {
            setLoading(false);
            setModalShow(false);
        }
    ...
    const handleCreateNew = () => {
        navigate('/admin/emak-kece/create');
    };

    const handleEdit = (id) => {
        navigate(`/admin/emak-kece/edit/${id}`);
    };
        ...
        <>
            ...
            <div className="overflow-x-auto my-32 px-6">
                <div className="flex justify-between mb-6">
                    <h2 className="text-2xl font-semibold text-gray-800">Emak Kece</h2>
                    <button
            ...
            <ConfirmationModal
                show={modalShow}
                title="Hapus Emak Kece"
                message="Apakah Anda yakin ingin menghapus Emak Kece ini?"
