...
const DonasiTable = () => {
    ...
    const fetchData = async () => {
        try {
            const response = await axios.get('http://localhost:3000/donasis');
    ...
    const handleCreateNew = () => {
        navigate('/admin/donasi/create'); 
    };

    const handleEdit = (id) => {
        navigate(`/admin/donasi/edit/${id}`);
    };
                    ...
                    {data.map((item) => (
                        <React.Fragment key={item.id}>
                            ...
                            >
                                ...
                                <td className="py-3 px-4">{item.judulDonasi}</td>
