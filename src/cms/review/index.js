...
const ReviewTable = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await axios.get('http://localhost:3000/reviews', { params: query });
    ...
    const handleDelete = async () => {
        setLoading(true);
        try {
            await axios.delete(`http://localhost:3000/reviews/${toBeDeletedId}`, {
                headers: {
                    Authorization: `Bearer ${token}`
                }
            });
            setToast({ show: true, type: 'success', message: 'Review deleted successfully!' });
            fetchData();
            setSelectedItem(null);
        } catch (error) {
            console.error('Error deleting review:', error);
            setToast({ show: true, type: 'error', message: 'Failed to delete review.' });
        } finally {
            setLoading(false);
            setModalShow(false);
        }
    ...
    const handleCreateNew = () => {
        navigate('/admin/review/create');
    };

    const handleEdit = (id) => {
        navigate(`/admin/review/edit/${id}`);
    };
                                        ...
                                        <td colSpan="5" className="py-4 px-6">
                                            <div className="flex justify-between">
                                                <div>
                                                    ...
                                                    <p><span className="font-semibold">Position:</span> {item.posisi}</p>
                                                    <p><span className="font-semibold">Description:</span> {item.deskripsi}</p>
