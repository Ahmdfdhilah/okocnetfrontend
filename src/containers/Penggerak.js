...
const Penggerak = () => {
    ...
    const fetchPenggerak = async () => {
        try {
            const response = await fetch('https://okocenet-72f35a89c2ef.herokuapp.com/penggerak-okoces');
            if (!response.ok) {
                    ...
                    <div class="grid mobile:grid-cols-1 mobile:gap-6 sm:grid-cols-2 md:grid-cols-3 lg:gap-8 lg:grid-cols-3">
                        ...
                        {penggerak.map((penggerak) => (
                            <div class="bg-white rounded-lg border drop-shadow-xl p-4">
                                <img className="min-h-60 max-h-60 min-w-40 mx-auto" src={`https://okocenet-72f35a89c2ef.herokuapp.com${penggerak.fotoPenggerak}`} alt={penggerak.namaPenggerak} />
                                <div class="px-1 py-4">
